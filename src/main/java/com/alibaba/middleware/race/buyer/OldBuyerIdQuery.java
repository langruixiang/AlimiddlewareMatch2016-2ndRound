...
//                RandomAccessFile hashRaf = RandomFile.randomFileMap.get(posinfo[0]);
    ...
    public static Iterator<Result> findOrdersByBuyer(long startTime, long endTime, String buyerid) {
        ...
        for (Order order : orders) {
            ...
            Good good = GoodCache.goodMap.get(order.getKeyValues().get("goodid").getValue());
            if (good == null) {
               good = GoodQuery.findGoodById(order.getKeyValues().get("goodid").getValue(), goodIdHashIndex);
            }
