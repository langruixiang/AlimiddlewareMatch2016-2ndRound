...
public class ExternalSortIndex {
    ...
    private TreeMap<TimeKey, Integer> index = new TreeMap<TimeKey, Integer>();

    public List<Integer> findPageNumber(long beginTime, long endTime) {
        List<Integer> ret = new ArrayList<Integer>();
        for (Map.Entry<TimeKey, Integer> entry : index.entrySet()) {
            if (entry.getKey().beginTime >= beginTime
                    && entry.getKey().beginTime <= endTime) {
    ...
    public void print() {
        for (Map.Entry<TimeKey, Integer> entry : index.entrySet()) {
            System.out.println(entry.getKey().toString() + "__"
                    + entry.getValue());
        }
    }
